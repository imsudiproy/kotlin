...
public extension ExportedKotlinPackages.oh.my.state {
    public final class State: KotlinRuntime.KotlinBase, KotlinRuntimeSupport._KotlinBridged {
        public var bytes: ExportedKotlinPackages.kotlin.ByteArray? {
            get {
                return { switch oh_my_state_State_bytes_get(self.__externalRCRef()) { case 0: .none; case let res: ExportedKotlinPackages.kotlin.ByteArray(__externalRCRef: res); } }()
            }
        }
        ...
        public init(
            ...
            oh_my_state_State_init_initialize__TypesOfArguments__Swift_UInt_Swift_Optional_ExportedKotlinPackages_kotlin_ByteArray___(__kt, bytes.map { it in it.__externalRCRef() } ?? 0)
        }
    }
}
