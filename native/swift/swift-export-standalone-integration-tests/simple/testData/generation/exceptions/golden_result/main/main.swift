...
public final class Object: KotlinRuntime.KotlinBase, KotlinRuntimeSupport._KotlinBridged {
    ...
    public init(
        ...
        var __error: UnsafeMutableRawPointer? = .none
    ...
    public init(
        ...
        super.init(__externalRCRef: __kt)
        var __error: UnsafeMutableRawPointer? = .none
        __root___Object_init_initialize__TypesOfArguments__Swift_UnsafeMutableRawPointer_Swift_Optional_KotlinRuntime_KotlinBase___(__kt, arg.map { it in it.__externalRCRef() } ?? .none, &__error)
        guard __error == .none else { throw KotlinError(wrapped: KotlinRuntime.KotlinBase(__externalRCRef: __error)) }
    }
}
public func throwing_fun_any() throws -> KotlinRuntime.KotlinBase {
    var _out_error: UnsafeMutableRawPointer? = .none
    let _result = __root___throwing_fun_any(&_out_error)
    guard _out_error == .none else { throw KotlinError(wrapped: KotlinRuntime.KotlinBase(__externalRCRef: _out_error)) }
...
public func throwing_fun_nullable() throws -> KotlinRuntime.KotlinBase? {
    ...
    guard _out_error == .none else { throw KotlinError(wrapped: KotlinRuntime.KotlinBase(__externalRCRef: _out_error)) }
    return { switch _result { case .none: .none; case let res: KotlinRuntime.KotlinBase(__externalRCRef: res); } }()
}
public func throwing_fun_nullable(
    arg: KotlinRuntime.KotlinBase?
) throws -> KotlinRuntime.KotlinBase? {
    var _out_error: UnsafeMutableRawPointer? = .none
    let _result = __root___throwing_fun_nullable__TypesOfArguments__Swift_Optional_KotlinRuntime_KotlinBase___(arg.map { it in it.__externalRCRef() } ?? .none, &_out_error)
    guard _out_error == .none else { throw KotlinError(wrapped: KotlinRuntime.KotlinBase(__externalRCRef: _out_error)) }
    return { switch _result { case .none: .none; case let res: KotlinRuntime.KotlinBase(__externalRCRef: res); } }()
}
